...
Parsable.prototype.stream = function(type, options) {
  ...
  if (!this._dataStream) {
    ...
    this._parserStream
        .pipe(this)
        .pipe(through2({ objectMode: true }, function (record, enc, callback) {
          // TODO understand why this is needed -- without it, the backpressure would prevent more than 16 or 48
          // records from being passed through.  It looks like the line below w/ PassThrough may have been
          // achieving this earlier, but with jsforce 1.5.0 it no longer seems to do the job.  Need to open a bug
          // upstream as well.
          this.push(record);
          this.read();
          callback();
